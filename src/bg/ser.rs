    ...
    if let json::JsonValue::Array(v) = &obj[key] {
        ...
        for (i, val) in v.iter().enumerate() {
            arr[i] = val.as_f64().ok_or_else(|| error!(JsonLoad,
                "Failed to parse '{}': element #{} of array '{}' is not a float", obj, i, key))?;
        }
        Ok(())
    } else {
        Err(error!(JsonLoad, "Failed to parse '{}': missing or incorrect array '{}'", obj, key))
    }
}
...
pub fn json_err(obj: &json::JsonValue, key: &str) -> Error {
    ...
    error!(JsonLoad, "Failed to parse '{}': missing or incorrect '{}' field!", obj_str, key)
}
