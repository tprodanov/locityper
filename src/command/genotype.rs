...
) -> crate::Result<()>
{
    ...
    let genotyping = if all_alns.reads().is_empty() {
        ...
        let genotyping = solve::solve(&data, bg_distr, &locus.out_dir, &mut rng)?;

        log::info!("Polishing predicted haplotypes");
        let best_gt = &genotyping.genotypes()[0];
        let best_pred = &genotyping.predictions()[0];
        crate::polish::discover::discover_variants(&best_gt, contigs, &data,
            best_pred.assgn_counts(), best_pred.attempts());
