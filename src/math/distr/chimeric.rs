      ...
      U: DiscretePmf + DiscreteCdf + Debug,
{
    ...
    ///
    /// Weights are set such that `distr1.pmf(partition) * rel_weight = distr2.pmf(partition)`.
    pub fn new(partition: u32, distr1: T, rel_weight: f64, distr2: U) -> Self {
        let val1 = rel_weight * distr1.ln_pmf(partition).exp();
        let val2 = distr2.ln_pmf(partition).exp();
    ...
    pub fn new_smooth(partition: u32, distr1: T, distr2: U) -> Self {
        Self::new(partition, distr1, 1.0, distr2)
    }
}
